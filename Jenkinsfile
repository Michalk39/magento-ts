pipeline {
    ...
    stages {
        ...
        stage('Test') {
            steps {
                sh 'docker run --rm --add-host=local.magento:172.17.0.1 --mount type=bind,src=${WORKSPACE},dst="/home/circleci" angular/ngcontainer -c "npm install && npm run webdriver-update-ci && xvfb-run npm test"'
            }
        }
        stage('Clean') {
            steps {
